...
$(TARGET): $(SRC) $(RAYLIB_A)
	$(CC) $(CFLAGS) -I$(RAYLIB_DIR) $(RAY_CFLAGS) -o $@ $(SRC) $(RAYLIB_A) $(RAY_LDLIBS) -lm -lpthread -ldl -lrt $(LDFLAGS)
...
.PHONY: all run clean clean-raylib android-init-signing android-debug android-release android-install android-install-release android-clean FORCE
