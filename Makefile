...
KRYON_TUI_SRCS := $(filter $(KRYON_DIR)/src/backend/tui_%.c,$(KRYON_SRCS))
KRYON_SRCS := $(filter-out $(KRYON_TUI_SRCS),$(KRYON_SRCS))
...
KRYON_SRCS += $(KRYON_TUI_SRCS) $(KRYON_NULL_BACKEND_C)
KRYON_NATIVE_CFLAGS := $(filter-out -DUI_WINDOW_HAVE_SDL,$(COMMON_CFLAGS)) -std=c99 $(RUNTIME_ASSET_CFLAGS) $(SYSTEM_THEME_CFLAGS) $(KRYON_NOTIFICATION_CPPFLAGS) $(KRYON_NOTIFICATION_CFLAGS)
DESKTOP_TRAY_CFLAGS :=
DESKTOP_TRAY_LDLIBS :=
else
$(error Unknown KRYON_BACKEND '$(KRYON_BACKEND)' (expected raylib, libdraw, or tui))
endif
...
tui:
	$(MAKE) KRYON_BACKEND=tui native
	INBE_NO_TRAY=1 "$(TARGET)"
