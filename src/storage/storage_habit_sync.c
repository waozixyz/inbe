...
storage_apply_sync_habits_json(const char *response_json)
{
    static const char *habits_sql =
        ...
        "counter_enabled,counter_target,sort_order,deleted_at,updated_at,weekdays,reminder_hour) "
        ...
        "                     0) AS INTEGER),"
        "       CAST(COALESCE(json_extract(value,'$.counter_target'),"
        "                     (SELECT counter_target FROM habits WHERE "
        "id=COALESCE(json_extract(value,'$.id'),'')),"
        "                     1) AS INTEGER),"
        ...
        " counter_enabled=excluded.counter_enabled,counter_target=excluded.counter_target,sort_order=excluded.sort_"
