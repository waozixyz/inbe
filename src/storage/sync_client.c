                                 ...
                                 char *out, size_t out_size)
{
    ...
    if(out == NULL || out_size == 0)
        return 0;
    out[0] = '\0';
    ok = AppendSyncBuffer(&buffer, first, first != NULL ? strlen(first) : 0) &&
         AppendSyncBuffer(&buffer, second, second != NULL ? strlen(second) : 0);
...
static SyncConfig
sync_kryon_config(const char *base_url, const SyncAccount *account)
{
    SyncConfig cfg;
...
int
sync_client_web_sync_poll(SyncResult *result, int *changed)
{
    int done;

    done = PollWebSync(result, changed);
    if(done && result != NULL && *result == SYNC_OK && changed != NULL)
