...
#include <string.h>
    ...
    !defined(PLATFORM_ANDROID) && !defined(__ANDROID__) && !defined(ANDROID)
...
sync_http_request(const char *method, const char *url, const char *body,
                  struct curl_slist *headers, SyncBuffer *response, long *status)
{
    CURL *curl;
    CURLcode res;
...
sync_fetch_challenge(const char *base_url, const char *user_id, char nonce_hex[65])
{
    ...
    strncat(url, user_id, sizeof(url) - strlen(url) - 1);
    ok = sync_http_request("GET", url, NULL, NULL, &response, &status);
    if(!ok || status != 200 ||
                 ...
                 const char *user_id, const char *body)
{
    ...
    ok = sync_http_request(method, url, body, headers, &response, &status);
