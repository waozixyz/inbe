...
void app_play_bell_cue(InbeApp *app, float volume) { (void)app; (void)volume; }
void app_close_modal(InbeApp *app) { (void)app; }
...
void practice_background_start(InbeApp *app, int id) { (void)app; (void)id; }
void practice_active_background_stop(InbeApp *app) { (void)app; }
void app_notifications_send_session_complete(InbeApp *app) { (void)app; }
void sync_habits_for_activity(InbeApp *app, int id) { (void)app; (void)id; }
...
void save_settings(InbeApp *app) { (void)app; settings++; }
int app_auto_sync(InbeApp *app) { (void)app; syncs++; return 1; }
void app_switch_screen(InbeApp *app, int screen) {
    app->inbe.screen = screen; switches++;
}
int data_discard_session(const char *path) {
    assert(strcmp(path, "session-1") == 0); writes++; return write_ok;
}
int data_save_session_checkin(const char *path, const InbeStorageSessionCheckin *checkin) {
...
int main(void) {
    ...
    assert(app.inbe.screen == InbeScreenResults && sessions == 1);
    assert(app.session_result.saved && app.session_result.primary_value == 60);
    meditation_advance_elapsed(&app, 60000);
    assert(sessions == 1);
    app.session_result.mood = 4;
    session_result_done(&app);
    assert(app.inbe.screen == InbeScreenStart && !app.session_result.active);
